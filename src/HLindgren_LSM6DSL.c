...
void lsm6dsl_open_i2c_slave_device(Lsm6dsl_I2cPortHandle_t *port_handle, uint8_t device_address,
...
static uint8_t data_rate_to_accel_odr_hp[] = {
    Lsm6dsl_OdrXlHp_12_5Hz,
    Lsm6dsl_OdrXlHp_26Hz,
    Lsm6dsl_OdrXlHp_52Hz,
    Lsm6dsl_OdrXlHp_104Hz,
    Lsm6dsl_OdrXlHp_208Hz,
    Lsm6dsl_OdrXlHp_416Hz,
    Lsm6dsl_OdrXlHp_833Hz,
    Lsm6dsl_OdrXlHp_1_66kHz,
    Lsm6dsl_OdrXlHp_3_33kHz,
    Lsm6dsl_OdrXlHp_6_66kHz,
};

static uint8_t data_rate_to_gyro_odr[] = {
    Lsm6dsl_OdrGyro_12_5Hz,
    Lsm6dsl_OdrGyro_26Hz,
    Lsm6dsl_OdrGyro_52Hz,
    Lsm6dsl_OdrGyro_104Hz,
    Lsm6dsl_OdrGyro_208Hz,
    Lsm6dsl_OdrGyro_416Hz,
    Lsm6dsl_OdrGyro_833Hz,
    Lsm6dsl_OdrGyro_1_66kHz,
    Lsm6dsl_OdrGyro_3_33kHz,
    Lsm6dsl_OdrGyro_6_66kHz,
};

static uint8_t data_rate_to_fifo_odr[] = {
    Lsm6dsl_OdrFifo_12_5Hz,
    Lsm6dsl_OdrFifo_26Hz,
    Lsm6dsl_OdrFifo_52Hz,
    Lsm6dsl_OdrFifo_104Hz,
    Lsm6dsl_OdrFifo_208Hz,
    Lsm6dsl_OdrFifo_416Hz,
    Lsm6dsl_OdrFifo_833Hz,
    Lsm6dsl_OdrFifo_1_66kHz,
    Lsm6dsl_OdrFifo_3_33kHz,
    Lsm6dsl_OdrFifo_6_66kHz,
};


void lsm6dsl_accel_init(Lsm6dsl_Device_t *device, Lsm6dsl_DataRate_e accel_data_rate)
{
    // Note assumes high performance mode (XL_HM_MODE = 0)
    uint8_t accel_odr = data_rate_to_accel_odr_hp[accel_data_rate];
    lsm6dsl_write_register(device, LSM6DSL_REG_CTRL1_XL, &accel_odr, 1);
}


void lsm6dsl_gyro_init(Lsm6dsl_Device_t *device, Lsm6dsl_DataRate_e gyro_data_rate)
{
    uint8_t gyro_odr = data_rate_to_gyro_odr[gyro_data_rate];
    lsm6dsl_write_register(device, LSM6DSL_REG_CTRL2_G, &gyro_odr, 1);
}


void lsm6dsl_fifo_init(Lsm6dsl_Device_t   *device,
                       Lsm6dsl_FifoMode_e fifo_mode,
...
Lsm6dsl_GyroData_t lsm6dsl_gyro_read(Lsm6dsl_Device_t *device)
{
    ...
    lsm6dsl_read_register(device, LSM6DSL_REG_OUTX_L_G, inbuf, LSM6DSL_GYRO_DATA_SIZE);
    
    uint16_t *buf16 = (uint16_t*)&inbuf;

    return (Lsm6dsl_GyroData_t){
        .x = buf16[0],
        .y = buf16[1],
        .z = buf16[2]
    };
...
Lsm6dsl_AccelData_t lsm6dsl_accel_read(Lsm6dsl_Device_t *device)
{
    ...
    lsm6dsl_read_register(device, LSM6DSL_REG_OUTX_L_XL, inbuf, LSM6DSL_ACCEL_DATA_SIZE);
    
    uint16_t *buf16 = (uint16_t*)&inbuf;

    return (Lsm6dsl_AccelData_t){
        .x = buf16[0],
        .y = buf16[1],
        .z = buf16[2]
    };
...
#define LSM6DSL_ERR_FIFO_DATABUF_RAN_OUT -2 // Error: Provided buffer to small to fit all data
                                            // available in FIFO
